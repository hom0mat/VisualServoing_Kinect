...
class PoseEstimator(Node):
    def __init__(self):
        ...
        self.declare_parameter('min_score_delta', 0.001)
        ...
        self.min_score_delta = float(self.get_parameter('min_score_delta').value)
    ...
    def cloud_callback(self, msg):
        ...
        # 2) Alineación inicial por centrado/escalado
        ...
        model_init = self.reference_pcd.translate((0,0,0), relative=False)
        model_init.scale(scale, center=cen_model)
        model_init.translate(cen_live-cen_model, relative=True)

        # 3) Comparar métodos y filtrar según threshold
        best_trans = None
        best_score = self.prev_score
        methods    = ['icp_alone']
        if self.use_ransac:
            methods.insert(0, 'ransac_icp')
        ...
        for m in methods:
            if m == 'ransac_icp':
                ...
                fpfh_src = o3d.pipelines.registration.compute_fpfh_feature(
                    src_down, o3d.geometry.KDTreeSearchParamHybrid(radius=self.voxel_size*5, max_nn=100))
                fpfh_tgt = o3d.pipelines.registration.compute_fpfh_feature(
                    tgt_down, o3d.geometry.KDTreeSearchParamHybrid(radius=self.voxel_size*5, max_nn=100))
                ...
                # Saltar si pocos inliers
                if len(ransac_res.correspondence_set) < self.min_inliers:
                    self.get_logger().info(f"RANSAC saltado: solo {len(ransac_res.correspondence_set)} inliers")
                    continue
                # ICP refinado
                icp_res = o3d.pipelines.registration.registration_icp(
                    model_init.transform(ransac_res.transformation), live,
                    self.icp_distance, np.identity(4),
            ...
            # Solo aceptar si mejora suficiente
            if score > best_score + self.min_score_delta:
                best_score = score
                best_trans = trans
                self.get_logger().info(f"Método {m} elegido, score mejorado: {best_score:.4f}")

        # 4) Publicar si hubo mejora real
        if best_trans is not None:
            ...
            t.transform.translation.x = float(best_trans[0,3])
            t.transform.translation.y = float(best_trans[1,3])
            t.transform.translation.z = float(best_trans[2,3])
            ...
            self.get_logger().info("No se publicó actualización: score no mejoró lo suficiente.")
